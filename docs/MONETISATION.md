...
**Upgrade** : à tout moment, en payant la différence (créez un Payment Link « Upgrade Essentiel → Pro » à 1 000 €, etc.). La progression est conservée.
...
3. **Conversion** : Payment Link en un clic, garantie 14 jours, facture automatique. Pas de compte à créer : la clé de licence suffit.
...
Quatre cohortes par an + upgrades + entreprise : un objectif de 250 à 300 k€ HT annuels est réaliste avec une audience de quelques milliers de professionnels du secteur. Le coût marginal est quasi nul (GitHub Pages est gratuit, Stripe prend ~1,5 % + 0,25 €) ; le coût réel est votre temps d'animation (lives, revues, coaching Elite).
...
- [ ] Un prix « early bird » (−30 %) pour les 10 premières places, via `priceBefore` et un Payment Link dédié.
...
| Clic Checkout → achat | Stripe | > 40 % |
| Taux de complétion (certificats délivrés) | déclaratif Discord + quiz | > 50 % |
| Remboursements | Stripe | < 5 % |
| Upgrades | Stripe | > 15 % des Essentiel sous 90 jours |
...
- **Clé de licence partagée par palier** en mode statique. Compensation : `api/activate.js` avec Lemon Squeezy pour des clés individuelles révocables ; et le contrat (CGV) qui interdit le partage. Dans les faits, le contenu B2B à ce prix est peu piraté ; la valeur est dans la communauté et les lives, qui ne se copient pas.
